$(document).ready(function () {
    ...
    */
    ...
    // Seleccionador de especialidades
    /*$('#ProfesionID').change(function () {
    $.get('/SAG2/Personal/Especialidad/' + $(this).val(), function (response) {
    var Especialidades = $.parseJSON(response);
    var ddlEspecialidades = $('#EspecializacionID');
    $('#EspecializacionID > option').remove();
    if (Especialidades.length == 0) {
    ddlEspecialidades.append($("<option />").val("1").text("Sin especialidad"));
    } else {
    ddlEspecialidades.append($("<option />").val("1").text("Sin especialidad"));
    for (i = 0; i < Especialidades.length; i++) {
    ddlEspecialidades.append($("<option />").val(Especialidades[i].Value).text(Especialidades[i].Text));
    }
    }
    });
    });
    */
    ...
    //
    /*
    if ($('#ProfesionID').val() != "1" && $('#ProfesionID').val() != undefined) {
        var especialidad = $('#ProfesionID').attr("especialidad");
        $.get('/SAG2/Personal/Especialidad/' + $('#ProfesionID').val(), function (response) {
            var Especialidades = $.parseJSON(response);
            var ddlEspecialidades = $('#EspecializacionID');
            $('#EspecializacionID > option').remove();
            if (Especialidades.length == 0) {
                ddlEspecialidades.append($("<option />").val("1").text("Sin especialidad").attr("selected", "selected"));
            } else {
                ddlEspecialidades.append($("<option />").val("1").text("Sin especialidad"));
                for (i = 0; i < Especialidades.length; i++) {
                    if (especialidad == Especialidades[i].Value) {
                        ddlEspecialidades.append($("<option selected=\"selected\" />").val(Especialidades[i].Value).text(Especialidades[i].Text));
                    } else {
                        ddlEspecialidades.append($("<option />").val(Especialidades[i].Value).text(Especialidades[i].Text));
                    }
                }
    ...
    */
    ...
    $('#RegionID').change(function () {
        $.get('/SAG2/Comuna/Comunas/' + $(this).val(), function (response) {
    ...
    if ($('#RegionID').val() != "" && $('#RegionID').val() != undefined) {
        ...
        $.get('/SAG2/Comuna/Comunas/' + $('#RegionID').val(), function (response) {
            var comunas = $.parseJSON(response);
            var ddlComunas = $('#ComunaID');
            $('#ComunaID > option').remove();
            if (comunas.length == 0) {
    ...
    $("#ComprobanteEgresoID").change(function () {
        $.get('/SAG2/Reintegros/Egreso/' + $('#ComprobanteEgresoID').val(), function (response) {
    ...
    $("#Bruto").change(function () {
        if ($('#Bruto').val() != "") {
            ...
            var porcentaje = new Number($('#Porcentaje').val());
            var retencion = bruto * porcentaje / 100;
    ...
    $("#Neto").change(function () {
        if ($('#Neto').val() != "") {
            ...
            var porcentaje = new Number($('#Porcentaje').val());
            var bruto = new Number(neto / (1 - (porcentaje / 100)));
    ...
    //$(".tipobeneficiario #tipoBeneficiario").change();
    ...
    $(".egreso #Origen").change(function () {
        ...
        if ($(this).val() == "ff") {
            ...
            *****/
            ...
            window.open("/SAG2/Egresos/FondoFijo", 'SeleccionCC', 'width=800,height=400');
            ...
            */
            ...
            if (personalID != 0) {
                window.open("/SAG2/DeudasPendientes/ListadoEgreso/?personalID=" + personalID, 'SeleccionDP', 'width=800,height=400');
            } else if (proveedorID != 0) {
                window.open("/SAG2/DeudasPendientes/ListadoEgreso/?proveedorID=" + proveedorID, 'SeleccionDP', 'width=800,height=400');
            } else {
                window.open("/SAG2/DeudasPendientes/ListadoEgreso/", 'SeleccionDP', 'width=300,height=800');
            }
            ...
            if (personalID != 0) {
                window.open("/SAG2/BoletasHonorarios/IngresarPopUp/?personalID=" + personalID, 'SeleccionBoletaHonorarios', 'width=800,height=600');
            } else if (proveedorID != 0) {
                window.open("/SAG2/BoletasHonorarios/IngresarPopUp/?proveedorID=" + proveedorID, 'SeleccionBoletaHonorarios', 'width=800,height=600');
            } else {
                window.open("/SAG2/BoletasHonorarios/IngresarPopUp/?rut=" + rut + "&dv=" + dv + "&beneficiario=" + beneficiario, 'SeleccionBoletaHonorarios', 'width=800,height=600');
            }
    ...
    */
    ...
    $(".egreso #GuardarLinea").click(function () {
        ...
        // Verificamos si hay saldo disponible:


        

        //var montoOriginal = 0;
        //if ($("#MontoOriginal").val() != "" && $("#MontoOriginal").val() != $("#Monto").val()) {
        ...
        if ($(".egreso #Origen").val() == "oo" || $(".egreso #Origen").val() == "dp" || $(".egreso #Origen").val() == "hs") {
            $.post("/SAG2/Egresos/GuardarLinea", $("#detalle form").serialize(),
            function (data) {
                if (data == "OK") {
                    // Si el monto fue guardado leemos utilidad para sumar detalles del egreso
                    $.get('/SAG2/Data/SumaDetalleEgreso', function(data) {
            ...
            //$.get('/SAG2/FondoFijo/GenerarEgreso/?DocumentoID=' + $(".egreso #DocumentoID").val(), function (response) {
            $.get('/SAG2/CajaChica/GenerarEgreso/?fondoFijoGrupoID=' + fondoFijoGrupoID, function (response) {
                $('body').css('opacity','0.25');
                if (response == "-1") {
    ...
    $(".egreso #BorrarLinea").click(function () {
        ...
        // Guardamos linea del formulario si opcion es otro
        if ($(".egreso #Origen").val() == "oo" || $(".egreso #Origen").val() == "dp" || $(".egreso #Origen").val() == "hs") {
            $.post("/SAG2/Egresos/BorrarLinea", $("#detalle form").serialize(),
            function (data) {
                if (data == "OK") {
                    $.get('/SAG2/Data/SumaDetalleEgreso', function(data) {
                        $('#Monto_Egresos').val(data);
    ...
    $("#ComprobanteEgreso").click(function () {
        window.open("/SAG2/Reintegros/ListadoDetalles/", 'Reintegros', 'width=800,height=400');
        return false;
    });

    $("#ComprobanteEgresoAlta").click(function () {
        window.open("/SAG2/Inventario/ListadoDetalles/", 'Inventario', 'width=800,height=400');
        return false;
    });
    ...
    $("#BodegaPeriodo").click(function () {
        ...
        var currSrc = "/SAG2/MovimientosBodega/ListarMovimientos?Periodo=" + periodo + "&Mes=" + mes;
    ...
    $("#BodegaMovimiento").click(function () {
        ...
        $.post("/SAG2/MovimientosBodega/GuardarMovimiento", $("form").serialize(),
            function (data) {
                if (data == "OK") {
                    $('body').css('opacity','0.25');
                    alert("Movimiento ingresado con con exito!");
                    $('body').css('opacity','1');
                    var currSrc = "/SAG2/MovimientosBodega/ListarMovimientos";
                    $("#iframeBodega").attr("src", currSrc);
                    $("#iframeBodega").show();
    ...
    $("#BorrarMovimientoBodega").click(function () {
        if ($("#MovimientosBodegaID").val() != undefined && $("#MovimientosBodegaID").val() != "") {
            if (confirm("Realmente desea borrar este movimiento?")) {
                $.post("/SAG2/MovimientosBodega/Delete", { mov: $("#MovimientosBodegaID").val() },
                function (data) {
                    if (data == "OK") {
                        $('body').css('opacity','0.25');
    ...
    $(".MovimientoBodega").click(function () {
        window.parent.document.getElementById("MovimientosBodegaID").value = $(this).attr("MovimientosBodegaID");
        window.parent.document.getElementById("Fecha").value = $(this).attr("Fecha");
        window.parent.document.getElementById("Observaciones").value = $(this).attr("Observaciones");
        window.parent.document.getElementById("ArticuloID").value = $(this).attr("ArticuloID");
        window.parent.document.getElementById("Cantidad").value = $(this).attr("Cantidad");
        window.parent.document.getElementById("NroDocumento").value = $(this).attr("NroDocumento");
        window.parent.document.getElementById("DocumentoID").value = $(this).attr("DocumentoID");
        if ($(this).attr("tipo_movimiento") == "E") {
            window.parent.document.getElementById("tipo_movimiento_s").removeAttribute("checked");
            window.parent.document.getElementById("tipo_movimiento_e").setAttribute("checked", "checked");
        } else {
            window.parent.document.getElementById("tipo_movimiento_e").removeAttribute("checked");
            window.parent.document.getElementById("tipo_movimiento_s").setAttribute("checked", "checked");
        }
    });
    ...
    $(".conciliar").click(function () {
        ...
        $.post("/SAG2/Banco/Conciliar", { detalleEgresoID: detalleEgresoID, movimientoID: movimientoID },
            function (data) {
                if (data == "OK" || data == "OK2") {
    ...
    $("#periodoPresupuesto").change(function () {
        window.location = '/SAG2/Presupuesto/Formulacion?Periodo=' + $(this).val();
        return false;
    });

    $("#periodoControl").change(function () {
        window.location = '/SAG2/Presupuesto/Control?Periodo=' + $(this).val();
        return false;
    });

    $("#periodoBalance").change(function () {
        window.location = '/SAG2/Presupuesto/Balance?Periodo=' + $(this).val();
        return false;
    });
    ...
    $("#lineaPeriodo").change(function () {
        ...
        window.location = '/SAG2/Presupuesto/LineaProteccion?Periodo=' + periodo + '&Linea=' + linea;
        return false;
    });
    ...
    $("#lineaPeriodoR").change(function () {
        ...
        window.location = '/SAG2/Presupuesto/LineaResponsabilidad?Periodo=' + periodo + '&Linea=' + linea;
        return false;
    });
    ...
    $("#exportarPresupuestoExcel").change(function () {
        ...
        if (value != "") {
            if (value == "Anual") {
                window.open('/SAG2/Presupuesto/Excel?Periodo=' + periodo, '', '');
            }else if (value == "1S") {
                window.open('/SAG2/Presupuesto/ExcelSemestre1?Periodo=' + periodo, '', '');
            } else if (value == "2S") {
                window.open('/SAG2/Presupuesto/ExcelSemestre2?Periodo=' + periodo, '', '');
            } else if (value == "2S") {
                window.open('/SAG2/Presupuesto/Excel', '', '');
            } else if (value == "1T") {
                window.open('/SAG2/Presupuesto/Excel', '', '');
            } else if (value == "2T") {
                window.open('/SAG2/Presupuesto/Excel', '', '');
    ...
    $("#exportarBalanceExcel").change(function () {
        ...
        if (value != "") {
            if (value == "Anual") {
                window.open('/SAG2/Presupuesto/ExcelBalance?Periodo=' + periodo, '', '');
            } else if (value == "1S") {
                window.open('/SAG2/Presupuesto/ExcelBalanceSemestre1?Periodo=' + periodo, '', '');
            } else if (value == "2S") {
                window.open('/SAG2/Presupuesto/ExcelBalanceSemestre2?Periodo=' + periodo, '', '');
            } else if (value == "2S") {
                window.open('/SAG2/Presupuesto/Excel', '', '');
            } else if (value == "1T") {
                window.open('/SAG2/Presupuesto/Excel', '', '');
    ...
    $("#excelBalanceLinea").click(function () {
        ...
        window.open('/SAG2/Presupuesto/LineaProteccionExcel?Periodo=' + lineaPeriodo + '&Linea=' + lineaProteccion, '', '');
        return false;
    });
    ...
    $("#excelBalanceResponsabilidad").click(function () {
        ...
        window.open('/SAG2/Presupuesto/LineaResponsabilidadExcel?Periodo=' + lineaPeriodoR + '&Linea=' + lineaResponsabilidad, '', '');
        return false;
    });
    ...
    */
    ...
    $(".excelBotonPeriodo").click(function () {
        ...
        if ($("#Desde").val() == undefined && $("#Hasta").val() == undefined) {
            window.open('/SAG2/Informes/Excel' + tipo +'?Mes='+$("#Mes").val()+'&Periodo='+$("#Periodo").val(), '', '');
        } else {
            window.open('/SAG2/Informes/Excel' + tipo +'?Desde='+$("#Desde").val()+'&Hasta='+$("#Hasta").val(), '', '');
        }
    ...
    $("#ImprimirReporteHH").click(function(){
        if ($("#Mes").val() == "" || $("#Periodo").val() == "") {
            ...
            window.open('/SAG2/BoletasHonorarios/ReporteImprimir?Mes='+$("#Mes").val()+'&Periodo='+$("#Periodo").val(), 'Reporte', 'width=750,height=400,scrollbars=yes,menubar=no,toolbars=no');
        }
        return false;
    });
    ...
    $(".BuscarPopUpPersonal").click(function(){
        if ($(".tipobeneficiario #tipoBeneficiario").val() == "1") {
            window.open("/SAG2/Personal/PopUp", 'BuscarPersona', 'width=750,height=400,scrollbars=yes,menubar=no,toolbars=no');
        } else if ($(".tipobeneficiario #tipoBeneficiario").val() == "2") {
            window.open("/SAG2/Proveedores/PopUp", 'BuscarProveedor', 'width=750,height=400,scrollbars=yes,menubar=no,toolbars=no');
        }
        return false;
    });
    ...
    $(".tipobeneficiario #DVBuscar").focusout(function(){
        ...
        if ($("#DVBuscar").val() != "" && $("#Rut").val() != "" && $(".tipobeneficiario #tipoBeneficiario").val() != "3") {
            if ($(".tipobeneficiario #tipoBeneficiario").val() == "1") {
                $.get('/SAG2/DeudasPendientes/Personal?rut=' + $("#Rut").val()+'&dv='+$("#DVBuscar").val(), function (response) {
                    var personal = $.parseJSON(response);
                    if (personal.length == 0) {
                        if (confirm("La persona ingresada no existe. Desea ingresarla ahora?")) {
                            window.open("/SAG2/Personal/IngresarPopUp?rut=" + $("#Rut").val()+'&dv='+$("#DVBuscar").val(), 'IngresarPersona', 'width=765,height=360');
                        } else {
                            return;
                        }
                    } else {
                ...
                $.get('/SAG2/DeudasPendientes/Proveedores?rut=' + $("#Rut").val()+'&dv='+$("#DVBuscar").val(), function (response) {
                    var proveedores = $.parseJSON(response);
                    if (proveedores.length == 0) {
                        if (confirm("El proveedor ingresado no existe. Desea ingresarlo ahora?")) {
                            window.open("/SAG2/Proveedores/IngresarPopUp?rut=" + $("#Rut").val()+'&dv='+$("#DVBuscar").val(), 'IngresarProveedor', 'width=765,height=400');
                        } else {
                            return;
                        }
                    } else {
    ...
    $("#ImprimirLibroBanco").click(function(){
        var periodo = $("#Periodo").val();
        var mes = $("#Mes").val();
        window.open("/SAG2/Imprimir/LibroBanco?Periodo=" + periodo+'&Mes='+mes, 'LibroBanco');
        return false;
    });
    ...
    $("#ExcelLibroBanco").click(function () {
        var periodo = $("#Periodo").val();
        var mes = $("#Mes").val();
        window.open("/SAG2/Imprimir/ExcelLibroBanco?Periodo=" + periodo + '&Mes=' + mes, 'LibroBanco');
        return false;
    });
    ...
    $("#ExportarConciliacion").click(function () {
        var periodo = $("#Periodo").val();
        var mes = $("#Mes").val();
        window.open("/SAG2/Banco/Exportar?periodo=" + periodo + '&mes=' + mes, 'LibroBanco');
        return false;
    }); 
    ...
    $("#ImprimirConciliacion").click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var periodo = $("#Periodo").val();
                var mes = $("#Mes").val();
                window.open("/SAG2/Imprimir/Conciliacion?Periodo=" + periodo + '&Mes=' + mes, 'Conciliacion');
                alert("asads");
            }
        });
        return false;
    ...
    */
    ...
    $(".MenuItem").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "/SAG2/Auditorias/Informe?Documentos=NO",
            data: dataString,
            cache: false,
            dataType: "json",
            success: function (data) { }
    ...
    $("#informeGeneral").click(function(){
        window.location = "/SAG2/Auditorias/ReporteGeneral/?DesdeMes=" + $("#DesdeMes").val() + "&DesdePeriodo=" + $("#DesdePeriodo").val() + "&HastaMes=" + $("#HastaMes").val() + "&HastaPeriodo=" + $("#HastaPeriodo").val();
    });

    $("#informeGrupal").click(function(){
        window.location = "/SAG2/Auditorias/ReporteGrupal/?DesdeMes=" + $("#DesdeMes").val() + "&DesdePeriodo=" + $("#DesdePeriodo").val() + "&HastaMes=" + $("#HastaMes").val() + "&HastaPeriodo=" + $("#HastaPeriodo").val() + "&tipoProyectoID=" + $("#TipoProyectoID").val();
    });
    ...
    */
    ...
    $(".PeriodoConfiguracionIndicadores").change(function(){
        var periodo = $(this).val();
        window.location = "/SAG2/Control/Estandares/?Periodo=" + periodo;
    });

    $(".PeriodoIndicadores").change(function(){
        var periodo = $(this).val();
        window.location = "/SAG2/Control/Indicadores/?Periodo=" + periodo;
    });
    ...
    */
    ...
    $(".imprimirDirectoAuditoria").click(function () {
        if ($(this).attr("programaID") == undefined) {
            ...
            window.open("/SAG2/Auditorias/ImprimirInforme?Programa=" + programa, 'Imprimir Informe Auditoría', '');
        }
    });
    ...
    $(".Inventario_Egreso").click(function(){
        window.open("/SAG2/Inventario/ListadoDetalles/", 'Egresos', 'width=800,height=400');
        return false;
    });
